...
std::pair<int, Levels> parse_command_line(int argc, char* argv[]) {
    int port;
    Levels level;
    switch (argc)
    {
...
void test_logger_writing(std::shared_ptr<SocketLogger> &file_log, const std::string &msg, Levels lvl){
...
int main(int argc, char* argv[]) {
    ...
    do{
        ...
        case '1': {
            ...
            else level = socket_log->get_level();
            std::thread handle_data(test_logger_writing, std::ref(socket_log), std::cref(msg), level);
            handle_data.join();
            break;
        }
        case '2': {
            ...
            std::cin >> menu2;
            switch(menu2){
                case '1' : socket_log->set_level(Levels::DEBUG); break;
                case '2' : socket_log->set_level(Levels::INFO); break;
                case '3' : socket_log->set_level(Levels::WARNING); break;
                case '4' : socket_log->set_level(Levels::ERROR); break;
                case '5' : socket_log->set_level(Levels::FATAL); break;
            }
            break;
        }
        }
